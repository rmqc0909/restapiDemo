package org.codingpedia.demo.rmqc.rest;
...
import org.codingpedia.demo.common.util.FileUtils;
import org.codingpedia.demo.rmqc.core.podcast.bean.Podcast;
import org.codingpedia.demo.rmqc.core.podcast.dao.PodcastDao;
...
public class PodcastRestService {
    ...
    @POST @Path("upload")
    @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
    public Response createFile(InputStream inputStream) throws IOException {
        FileUtils.copyInputStreamToFile(inputStream, new File("E:\\test.txt"));
        return Response.status(204).build();
    }
    ...
    public Response findById(@PathParam("id") Long id) {
        logger.info("get podcast with the id logger :{}", id);
        log.info("get podcast with the id log :{}", id);
        Podcast podcastById = podcastDao.getPodcastById(id);
        if(podcastById != null) {
            log.info("get podcast with the id:{}", id);
